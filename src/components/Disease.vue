<template>
  <div class="hello">
    <div style="margin-left:35%;">
      ...
      <div class="row" style="margin-left : 20%;">คำค้นหายอดนิยม :
        <div class="mr-2" :key="key" v-for="(hit, key) in show">
          <b-badge variant="secondary">{{hit.search}}</b-badge>
        </div>
      </div>
    </div>
    <div>
      ...
      <div class="container">
        <div class="row">
          <div
            ...
            :key="key"
            v-for="(subadddisease, key) in subadds"
          ...
          >
            <div class="col-sm">
              <b-card
                title
                img-src
                v-bind:src="subadddisease.addpicturedisease"
                img-alt="Image"
                img-top
                tag="article"
              ...
              >
                ...
                <b class="card-text h4" >{{subadddisease.adddisease.slice(0, 50)}}</b>
                <div class="card-text" v-html="subadddisease.adddisease2.slice(0, 50)"></div>
                <b-button
                  @click="selectDis(subadddisease.adddisease)"
                  variant="primary"
      ...
      <div class="container">
        <div class="row">
          <div
            class="column is-one-third"
            :key="key"
            v-for="(subadddisease, key) in showData"
            v-if="showData.length > 0"
          >
            <div class="col-sm">
              <b-card
              ...
              >
                ...
                <p class="card-text">{{subadddisease.adddisease.slice(0, 50)}}</p>
                <p class="card-text">{{subadddisease.adddisease2.slice(0, 50)}}</p>
                <b-button
                  @click="selectDis(subadddisease.adddisease)"
                  variant="primary"
                  class="btn btn-outline-info"
    ...
    filter(Search) {
      if (Search.length > 0) {
        this.showData = this.subadds.filter(user => {
          if (
            user.adddisease.toString().indexOf(Search) >= 0 ||
